...
The "Physics Kernel" is written in TypeScript using `@thi.ng/shader-ast`. This AST is compiled to a GLSL Vertex Shader. During the `update` pass, the GPU runs this shader and writes the results directly back into a Vertex Buffer (VBO) without returning to CPU. On the next frame, that VBO is used as the input, creating a feedback loop.
